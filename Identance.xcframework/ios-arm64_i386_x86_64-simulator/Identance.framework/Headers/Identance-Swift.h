...
#ifndef IDENTANCE_SWIFT_H
#define IDENTANCE_SWIFT_H
...
/// Used to setup merchant related localization keys.
/// Currently have meaning only in <code>allStages</code> mode. In <code>singleStage</code> mode you could setup empty object:
/// <code>config.localizationKeys = .empty</code>
@property (nonatomic, strong) ZNLocalizationKeys * _Null_unspecified localizationKeys;
...
typedef SWIFT_ENUM_NAMED(NSInteger, ZNDossierSubmitStatus, "DossierSubmitStatus", open) {
/// User close verification without any changes to Dossier
  ZNDossierSubmitStatusCancelled = 0,
/// User Dossier was changed during current verification session
  ZNDossierSubmitStatusDossierSubmitted = 1,
/// User Dossier is in rejected state
  ZNDossierSubmitStatusDossierRejected = 2,
};


@class ZNLocalizationStageDescription;

/// Class which encapsulates localization keys related with particular merchant
SWIFT_CLASS_NAMED("LocalizationKeys")
@interface ZNLocalizationKeys : NSObject
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) ZNLocalizationKeys * _Nonnull empty;)
+ (ZNLocalizationKeys * _Nonnull)empty SWIFT_WARN_UNUSED_RESULT;
/// Stage description keys. MUST contain properly initialized objects in case merchant use <code>allStages</code> mode.
/// In <code>singleStage</code> only mode you could set empty array.
@property (nonatomic, copy) NSArray<ZNLocalizationStageDescription *> * _Nonnull stageDescriptions;
- (nonnull instancetype)initWithStageDescriptions:(NSArray<ZNLocalizationStageDescription *> * _Nonnull)stageDescriptions OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

enum ZNVerificationStage : NSInteger;

SWIFT_CLASS_NAMED("LocalizationStageDescription")
@interface ZNLocalizationStageDescription : NSObject
/// Stage to which localization keys must be linked
@property (nonatomic) enum ZNVerificationStage stage;
/// Value localized by this key used in stage cell on stage list in <code>allStages</code> mode.
@property (nonatomic, copy) NSString * _Nonnull shortDescriptionKey;
/// Value localized by this key used on stage details screen in <code>allStages</code> mode. Value supports Markdown styles.
@property (nonatomic, copy) NSString * _Nonnull fullDescriptionKey;
- (nonnull instancetype)initWithStage:(enum ZNVerificationStage)stage shortDescriptionKey:(NSString * _Nonnull)shortDescriptionKey fullDescriptionKey:(NSString * _Nonnull)fullDescriptionKey OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
typedef SWIFT_ENUM_NAMED(NSInteger, ZNVerificationStage, "VerificationStage", open) {
  ZNVerificationStageIdentity = 0,
  ZNVerificationStageAddress = 1,
  ZNVerificationStageEnhanced = 2,
  ZNVerificationStageCorporate = 3,
  ZNVerificationStageQuestionnaire = 4,
};
